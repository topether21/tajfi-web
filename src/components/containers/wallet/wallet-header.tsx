...
const ActionButton = ({ Icon, label, href, setActiveAction, isActive }: { Icon: React.ElementType; label: string; href: string; setActiveAction: React.Dispatch<React.SetStateAction<string | null>>; isActive: boolean }) => {
  return (
    ...
    <div className="hidden md:block bg-background text-white px-8">
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-around mt-8 relative">
          <ActionButton Icon={Send} label="Send" href="/wallet/send" isActive={activeAction === 'Send'} setActiveAction={setActiveAction} />
          <ActionButton Icon={Download} label="Receive" href="/wallet/receive" isActive={activeAction === 'Receive'} setActiveAction={setActiveAction} />
          <ActionButton Icon={History} label="History" href="/wallet/history" isActive={activeAction === 'History'} setActiveAction={setActiveAction} />
          <ActionButton Icon={Wallet} label="Marketplace" href="/wallet/marketplace" isActive={activeAction === 'Marketplace'} setActiveAction={setActiveAction} />
        </div>
        {loading || !balances ? (
